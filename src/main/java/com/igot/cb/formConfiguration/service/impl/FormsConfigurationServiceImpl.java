...
public class FormsConfigurationServiceImpl implements FormsConfigurationService {
    ...
    public ApiResponse createFormConfig(Map<String, Object> request, String token) {
        log.info("FormsConfigurationServiceImpl::createFormConfig: creating form config");
        ...
        try {
            ...
            Map<String, Object> requestData = (Map<String, Object>) request.get(Constants.Parameters.REQUEST);
            FormConfigResolutionContext ctx = buildContextFromCriteria(requestData);
            String requestedName = requestData.get(Constants.NAME).toString();
            // Same matching the read path uses — if a row already satisfies this criteria AND
            // carries the same name, this is the same logical row: call the update API instead of
            // inserting a duplicate. A criteria match under a different name is a different row.
            Optional<FormConfigurationEntity> existingOpt = formConfigRuleEngine.resolve(ctx);
            if (existingOpt.isPresent() && requestedName.equals(existingOpt.get().getName())) {
                log.info("FormsConfigurationServiceImpl::createFormConfig: matching form config already exists, delegating to updateFormConfig");
                return updateFormConfig(request, token);
            }

            FormConfigurationEntity entity = new FormConfigurationEntity();
            applyRequestToEntity(entity, requestData, userDetails.getUserId(), true);
            formConfigurationRepository.save(entity);

            response.put(Constants.CREATED_ON, entity.getCreatedAt());
            response.setResult(buildSaveResult(entity));
            response.setResponseCode(HttpStatus.OK);
            response.getParams().setStatus(Constants.SUCCESSFUL);
        } catch (Exception e) {
            log.error("Failed to createFormConfig: {}", e.getMessage(), e);
            ProjectUtil.returnErrorMsg("Failed to createFormConfig: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR, response, Constants.FAILED);
            return response;
        }
        return response;
    }
    ...
    @Override
    public ApiResponse updateFormConfig(Map<String, Object> request, String token) {
        log.info("FormsConfigurationServiceImpl::updateFormConfig: updating form config");
        ...
        try {
            ...
            if (existingOpt.isEmpty()) {
                ProjectUtil.returnErrorMsg("FormConfig data not found for the given criteria", HttpStatus.NOT_FOUND, response, Constants.FAILED);
                return response;
            }

            FormConfigurationEntity entity = existingOpt.get();
            applyRequestToEntity(entity, requestData, userDetails.getUserId(), false);
            formConfigurationRepository.save(entity);
            ...
            log.error("Failed to updateFormConfig: {}", e.getMessage(), e);
    ...
    /**
     * Builds the same {@link FormConfigResolutionContext} shape the read path resolves against,
     * but sourced from the request's own {@code criteria} block rather than the caller's token —
     * create/update describe who a row is *for*, they don't ask "who am I".
     */
    private FormConfigResolutionContext buildContextFromCriteria(Map<String, Object> requestData) {
        String type = requestData.get(Constants.TYPE).toString();
        String subtype = requestData.get(Constants.SUBTYPE).toString();
        String portal = requestData.get(Constants.PORTAL).toString();
        Double clientVersion = Double.valueOf(requestData.get(Constants.CLIENT_VERSION).toString());
        ...
        FormConfigResolutionContext.FormConfigResolutionContextBuilder ctxBuilder = FormConfigResolutionContext.builder()
                .type(type).subtype(subtype).portal(portal).clientVersion(clientVersion);

        Object designationValue = criteria.get(Constants.DESIGNATION);
        if (ObjectUtils.isNotEmpty(designationValue)) {
            List<String> designations = ((List<?>) designationValue).stream()
                    .map(String::valueOf)
                    .collect(Collectors.toList());
            ctxBuilder.designations(designations)
                    .ministryOrStateType(String.valueOf(criteria.get(Constants.ROOTORG)));
        } else {
            ctxBuilder.roles(List.of(String.valueOf(criteria.get(Constants.ROLE))))
                    .rootOrg(String.valueOf(criteria.get(Constants.ROOTORG)));
        }
        return ctxBuilder.build();
    }

    private void applyRequestToEntity(FormConfigurationEntity entity, Map<String, Object> requestData, String userId, boolean isNew) {
        String formattedCurrentTime = getFormattedCurrentTime(new Timestamp(System.currentTimeMillis()));
        if (isNew) {
            entity.setCreatedAt(formattedCurrentTime);
            entity.setCreatedBy(userId);
        }
        entity.setUpdatedAt(formattedCurrentTime);
        entity.setUpdatedBy(userId);
        entity.setName(requestData.get(Constants.NAME).toString());
        entity.setType(requestData.get(Constants.TYPE).toString());
        entity.setSubtype(requestData.get(Constants.SUBTYPE).toString());
        entity.setPortal(requestData.get(Constants.PORTAL).toString());
        entity.setClientVersion(Double.valueOf(requestData.get(Constants.CLIENT_VERSION).toString()));
        if (requestData.containsKey(Constants.CRITERIA)) {
            entity.setCriteria(objectMapper.valueToTree(requestData.get(Constants.CRITERIA)));
        }
        if (requestData.containsKey(Constants.DATA)) {
            entity.setData(objectMapper.valueToTree(requestData.get(Constants.DATA)));
        }
    }

    private Map<String, Object> buildSaveResult(FormConfigurationEntity entity) {
        Map<String, Object> result = new HashMap<>();
        ...
        result.put(Constants.CLIENT_VERSION, entity.getClientVersion());
        if (entity.getCriteria() != null) {
            result.put(Constants.CRITERIA, objectMapper.convertValue(entity.getCriteria(), Map.class));
        }
